...
class MathServiceTester:
    ...
    async def load_test(self, operations: int = 10) -> None:
        ...
        for i in range(operations):
            ...
            if i % 3 == 0:
                task = self.client.post("/api/v1/power/", json={"base": 2, "exponent": i % 10})
            elif i % 3 == 1:
                task = self.client.get(f"/api/v1/fibonacci/{i % 20}")
            else:
                task = self.client.post("/api/v1/factorial/", json={"n": i % 10})
        ...
        print(f"Load test completed: {success_count}/{operations} successful in {duration:.2f}s")
